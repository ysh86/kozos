...
static struct consreg {
    ...
    uint32_t dummy_for_alignment[2];
} consreg[CONSDRV_DEVICE_NUM];

/* Interrupts should be disabled
   if you call send_char() or send_string() from threads. */
...
static void send_string(struct consreg *cons, char *str, size_t len)
{
    size_t i;
    for (i = 0; i < len; i++) {
        if (str[i] == '\n') {
...
static int consdrv_command(struct consreg *cons, kz_thread_id_t id,
                           int index, size_t size, char *command)
{
    switch (command[0]) {
